...
using OpenAI (GPT) API via Responses API.

Input JSONL expected fields per line:
  - id (str)
  - image_path (str)
...
Usage examples:
  # V2
  python build_set_openai.py --version v2 \
    --input /workspace/RCLIP/docci_pairs_100.jsonl \
    --output /workspace/rclip/rclip_v2.jsonl \
    --model gpt-4.1-mini \
    --resume \
    --workers 4 \
    --qps 2.0
...
def append_jsonl_threadsafe(path: str, obj: Dict[str, Any], lock: threading.Lock) -> None:
...
# -----------------------------
# Rate limiter (QPS) for parallel workers
# -----------------------------
class QPSLimiter:
...
def call_openai_once(
    ...
    """
    Uses Responses API with a multimodal input:
    - system instruction
    - user content: image + text prompt
    """
    ...
    # SDK returns a structured object; easiest is .output_text if available
    out = getattr(resp, "output_text", None)
    if isinstance(out, str) and out.strip():
        return out.strip()

    # fallback: try to reconstruct from output
    try:
        chunks = []
...
def generate_with_retry(
    ...
    """
    Get a complete JSON with all tags. If parse fail / invalid, retry with repair suffix.
    """
    repair_suffix = """
...
"""
    ...
    for attempt in range(retries + 1):
        try:
            ...
            raw = call_openai_once(
                client=client,
                model=model,
                image_data_url=image_data_url,
                prompt_text=prompt_text + (repair_suffix if attempt > 0 else ""),
                max_output_tokens=max_output_tokens,
                temperature=temperature,
            )
            last_raw = raw
            ...
            # exponential backoff with jitter (recommended for 429 etc.)
            if attempt < retries:
    ...
    raise RuntimeError(f"Failed after retries. Last error/output head:\n{head}")
...
def process_one(
    ...
    if not isinstance(img_path, str) or not os.path.exists(img_path):
        return {"id": sid, "image_path": img_path, "error": f"missing image: {img_path}"}
    if not isinstance(txt, str) or not txt.strip():
        return {"id": sid, "image_path": img_path, "error": "missing text field"}
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser("Build DOCCI sets for V1/V2/V3 from (image + text) using OpenAI API.")
    ap.add_argument("--version", required=True, choices=["v1", "v2", "v3"])
    ap.add_argument("--input", default="/workspace/RCLIP/docci_pairs_100.jsonl")
    ...
    # Keep this reasonable; your JSON is small
    ...
    ap.add_argument("--workers", type=int, default=1, help="Thread workers for parallel generation.")
    ap.add_argument("--qps", type=float, default=0.0, help="Global QPS limit across workers (0=disabled).")
    ...
    # OpenAI client
    # Prefer env OPENAI_API_KEY; allow override
    api_key = args.api_key or os.environ.get("OPENAI_API_KEY")
    if not api_key:
        raise ValueError("Missing API key: set OPENAI_API_KEY or pass --api-key")
    client = OpenAI(api_key=api_key)

    limiter = QPSLimiter(args.qps) if args.qps and args.qps > 0 else None
    write_lock = threading.Lock()

    items = []
    for it in read_jsonl(args.input):
        sid = it.get(args.id_field)
        if isinstance(sid, str) and sid and (not args.resume or sid not in done):
            items.append(it)

    pbar = tqdm(total=len(items), dynamic_ncols=True)

    if args.workers <= 1:
        for it in items:
            try:
                out_obj = process_one(it, args, prompt_tpl, tags, client, limiter)
            except Exception as e:
                sid = it.get(args.id_field)
                out_obj = {"id": sid, "image_path": it.get(args.image_field), "error": str(e)}
            append_jsonl_threadsafe(args.output, out_obj, write_lock)
            pbar.update(1)
        pbar.close()
        return
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        futs = []
        for it in items:
            futs.append(ex.submit(process_one, it, args, prompt_tpl, tags, client, limiter))

        for fut, it in zip(as_completed(futs), items):
            try:
                out_obj = fut.result()
            except Exception as e:
                sid = it.get(args.id_field)
